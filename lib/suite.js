...
Suite.prototype.countOnlysRecursive = function(children, count) {
    children.itensForEach((child) => {
        if(child.type() === 'Test') {
            if(child.isOnly)
//               || (!child.isSkip && !this.hasOnlyRecursive(child.parent.children)))
                count.tests++;
        }else if(child.type() === 'Suite') {
            if(this.hasOnlyRecursive(child.children) || child.isOnly)
//                || (!child.isSkip && !this.hasOnlyRecursive(child.parent.children)))
                count.suites++;
            this.countOnlysRecursive(child.children, count);
        }
    });
}

Suite.prototype.countSkipRecursive = function(children, count) {
    children.itensForEach((child) => {
        if(child.type() === 'Test') {
            if(child.isOnly && !child.isSkip 
               || !child.isSkip && !this.hasOnlyRecursive(child.parent.children))
                count.tests++;
        }else if(child.type() === 'Suite') {
            if(child.isOnly && !child.isSkip 
               || !child.isSkip && !this.hasOnlyRecursive(child.parent.children))
                count.suites++;
            this.countSkipRecursive(child.children, count);
        }
    });
}
