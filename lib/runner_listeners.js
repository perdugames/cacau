...
function createRunnerListeners(runner) {
    ...
    */
    ...
    runner.eventBus.once('runStart', function() {
        if(result.suitesOnlys || result.testsOnlys) {
            const count = {tests: 0, suites: 0};
            runner.rootSuite.countOnlysRecursive(runner.rootSuite.children, count);
            result.suites = count.suites;
            result.tests = count.tests;
        }
        if(result.suitesSkips || result.testsSkips) {
            const count = {tests: 0, suites: 0};
            runner.rootSuite.countSkipRecursive(runner.rootSuite.children, count);
            console.log(count)
            result.suites = count.suites;
            result.tests = count.tests;
        }
